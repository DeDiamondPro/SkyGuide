...
class Condition(private val condition: String) {
    ...
    fun evaluate(x: Float, y: Float, z: Float): Boolean {
        ...
        if (or) return evaluated || nextCondition!!.evaluate(x, y, z)
        return evaluated && nextCondition!!.evaluate(x, y, z)
    }

    private fun parseCondition() {
        val cleanedCondition = condition.lowercase(Locale.getDefault()).replace(" ", "")
        val match = regex.matchEntire(cleanedCondition) ?: return
    ...
    companion object {
        private val regex = Regex("(?<variable>[x-z])(?<operator>[<>])(?<value>[0-9]+)(?<chain>[|&]{2})?(?<next>.+)?")
    }
